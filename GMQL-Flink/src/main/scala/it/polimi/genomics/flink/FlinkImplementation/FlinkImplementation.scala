    ...
    if(mo.intermediateResult.isDefined){
      ...
      val res =
        mo match {
          ...
          case IRProjectMD(projectedAttributes: Option[List[String]], metaAggregator: Option[MetaExtension], _,inputDataset: MetaOperator) => ProjectMD(this, projectedAttributes, metaAggregator, inputDataset, env)
