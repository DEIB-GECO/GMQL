  ...
  def extractor(dataset : RDD[(Long, String, Long, Long, Char, Long, Array[GValue])], binSize : Long) = {
    dataset.flatMap{x =>
      ...
      if(startBin==stopBin) {
        ...
        val map_start  = ((x._2,startBin,x._1,x._5),HashMap((x._3 - startBin * binSize +1).toInt -> 1, binSize.toInt -> -1))
        val map_stop   = ((x._2,stopBin, x._1,x._5),HashMap(( x._4 - stopBin  * binSize +1).toInt -> -1, 0 -> 1))
