  ...
  def implementation(): Unit = {
    ...
    try {
      for (variable <- to_be_materialized) {
        ...
        val extension =
          if (outputFormat == GMQLSchemaFormat.GTF) StoreGTFRD.extension
          else if (outputFormat == GMQLSchemaFormat.TAB) StoreTABRD.extension
          else throw new Exception("Unknown format")
        val names = Store.sampleNames(metaRDD, extension, sc)

        // Compute Profile and store into xml files (one for web, one for optimization)
        val profile = Profiler.profile(regions = regionRDD, meta = metaRDD, sc = sc, Some(names))
