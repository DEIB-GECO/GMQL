    ...
    if (mo.intermediateResult.isDefined) {
      ...
      val res =
        mo match {
          ...
          case x: IRStoreFedMD =>
            val splitId = x.annotations.collectFirst {
              case SPLIT_ID(id) => id.toString
            }
            StoreFed.storeMeta(this, splitId.getOrElse("menouno") , x.in, sc)
          case x: IRReadFedMD =>
            val splitId = x.annotations.collectFirst {
              case SPLIT_ID(id) => id.toString
            }
            ReadFed.readMeta(splitId.getOrElse("menouno"), sc)
        }
      ...
      val res =
        ro match {
          ...
          case x: IRStoreFedRD =>
            ...
            StoreFed.storeRegion(this, splitId.getOrElse("menouno") , x.in, sc)
          case x: IRReadFedRD =>
            val splitId = x.annotations.collectFirst {
              case SPLIT_ID(id) => id.toString
            }
            ReadFed.readRegion(splitId.getOrElse("menouno"), sc)
        }
