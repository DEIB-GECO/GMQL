...
//    storeSchema(generateSchema(input),path)
    input
  }
...
//    if (schema.isDefined) {
      ...
      if ((new java.io.File("/" + parentDir.getPath)).exists())
      ...
      else if (Utilities.getInstance().MODE == "LOCAL")
        scala.xml.XML.save(Utilities.getInstance().RepoDir + Utilities.USERNAME + "/schema/" + parentDir.getPath + ".schema", schemaHeader, ENCODING, true, null)
      else {
        ...
        val hdfsPath = Utilities.getInstance().HDFSRepoDir + Utilities.USERNAME + "/schema/" + parentDir.getPath + ".schema"
        scala.xml.XML.save(localPath, schemaHeader, ENCODING, true, null)
        logger.info("\n\n"+localPath+"\n\n")
        Utilities.getInstance().copyfiletoHDFS(localPath, hdfsPath)
      }
