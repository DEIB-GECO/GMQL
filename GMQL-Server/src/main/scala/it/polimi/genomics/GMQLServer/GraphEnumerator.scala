      ...
      if(list.size.equals(0)){
        ...
        val children : List[IROperator] = {
          list.head match {
            ...
            case IRGenometricJoin(metajoinCondition : Option[MetaJoinOperator], joinCondition : List[JoinQuadruple], regionBuilder : RegionBuilder, join_on_attributes : Option[List[(Int,Int)]], leftDataset : RegionOperator, rightDataset : RegionOperator) => List(leftDataset, rightDataset) ++ { if(metajoinCondition.isDefined) List(metajoinCondition.get) else List()}
