...
class FederatedImplementation extends Implementation with Serializable {
  ...
  def recursiveCall(gigi: ExecutionDAG): Unit = {
    if (gigi.hasDependencies)
      gigi.getDependencies.foreach(recursiveCall)
    val irVars: List[IRVariable] = try {
      gigi.dag.flatMap(x => x.toVariables(binningPar))
    } catch {
  ...
  def implementation(): Unit = {
    ...
    executionDAGs.roots.foreach(recursiveCall)
  }
