...
import it.polimi.genomics.scidb.utility.{DebugUtils, SchemaUtils}
...
object GmqlSciImporter
{
  ...
  def optimizeRD(name:String,
                 RD:SciStoredArray,
                 script:SciScript) : Unit =
  {
    val rankA = Attribute("rank", STRING)
    val rankVA = Attribute("rankid", INT64)

    // ------------------------------------------------
    // ------------------------------------------------

    val RANK = RD.reference()
      .apply(
        (rankA.e, OP(
          OP(
            OP(C_STRING(C_INT64(OP(GDS.left_D.e, "/", V(GDS.chunk_size)))), "+", V("_")), "+",
            OP(C_STRING(C_INT64(OP(GDS.left_D.e, "/", V(GDS.chunk_size)))), "+", V("_"))
          ), "+", C_STRING(GDS.enumeration_D.e)))
      )
      .rank(rankA.e)(GDS.sid_D.e, GDS.chr_D.e)
      .apply((rankVA.e, C_INT64(A("rank_rank"))))
      .project(rankVA.e)

    // ------------------------------------------------
    // ------------------------------------------------

    val RANKED = RD.reference()
      .cross_join(RANK, "SOURCE", "RANK")(
        GDS.regions_dimensions_D.map(dim => (dim.alias("SOURCE").e, dim.alias("RANK").e)):_*
      )
      .cast(
        List(GDS.sid_D, GDS.chr_D, GDS.left_D, GDS.right_D, GDS.strand_D, GDS.enumeration_D.label("old")),
        RD.getAttributes() ::: List(GDS.enumeration_D.toAttribute())
      )
      .redimension(GDS.regions_dimensions_D, RD.getAttributes(), true)
      .store(name + "_RD")                                                  // store the final result

    // ------------------------------------------------
    // Result -----------------------------------------

    script.addStatement(RANKED)
    script.addStatement(new SciOperation("remove", RD.getAnchor()))
  }
    ...
    : (SciStoredArray) =
  {
    ...
    val RD_RAW = samples
      ...
      .map(item => (item._1.apply(
        (A(GDS.sid_D.name),V(item._2))
      ), item._2))
    ...
    val RD = RD_PREPARED.reference()
      ...
      .store(name + "_RD_NOOPTIMIZED")                                                  // store the final result
