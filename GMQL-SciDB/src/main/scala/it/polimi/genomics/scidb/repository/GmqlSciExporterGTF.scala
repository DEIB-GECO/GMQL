...
import it.polimi.genomics.scidbapi.{SciAbstractArray, SciStoredArray, SciArray}
...
object GmqlSciExporterGTF
{
  ...
  def exportDS(name:String,
               path:String,
               columns:List[(String, PARSING_TYPE)],
               format:String = "tsv")
  : Unit =
  {
    // ------------------------------------------------
    ...
    val schema = columns.map(item => SchemaUtils.toAttribute(item._1, item._2))
    ...
    val RD_exported = samples
      ...
      .map(item => (
        item._1
          .apply(
            (A("DATA"),V("DATA")),
            (A("REGION"),V("GMQL_Region")),
            (A("SCORE"), V("100")),
            (A("POINT"), V(".")),
            (A("VALUE"),
                RD.getAttributes()
                  .map(a =>
                    OP(OP(V(a.name.toUpperCase + " \""), "+", C_STRING(a.e)), "+", V("\"; "))
                  )
                  .reduce((a1:Expr,a2:Expr) => OP(a1,"+",a2))
              ))
          .project(
            A("CHR"), A("DATA"), A("REGION"), A("LEFT"), A("RIGHT"), A("SCORE"), A("STRAND"), A("POINT"), A("VALUE")
          )
      ...
      .map(item => item._1.save(                                                                        // save the sample regiondata
        path +"/"+ name +"_"+ ("%0"+digits+"d").format((item._2))+".gtf",
        -2, format
      ))
