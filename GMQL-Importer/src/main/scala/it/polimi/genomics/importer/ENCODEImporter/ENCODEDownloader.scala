...
object ENCODEDownloader extends GMQLDownloader {
      ...
      if (urlExists(indexAndMetaUrl)) {
        downloadFileFromURL(
          ...
          source.outputFolder + File.separator + dataset.outputFolder +
            File.separator + "Downloads" + File.separator + dataset.outputFolder + ".tsv")
  ...
  private def downloadFilesFromMetadataFile(source: GMQLSource, dataset: GMQLDataset): Unit = {
    ...
    val log = new FileLogger(source.outputFolder + File.separator + dataset.outputFolder + File.separator + "Downloads")
    ...
    Source.fromFile(
      ...
      if (urlExists(fields(url))) {
        if (log.checkIfUpdate(filename, fields(url), fields(originSize), fields(originLastUpdate))) {
          ...
          downloadFileFromURL(fields(url), source.outputFolder + File.separator + dataset.outputFolder +
